...
const App = () => {
  ...
  useEffect(() => {
    if (!planCargando) {
      if (seleccionados.length > limiteProductos()) {
        setSeleccionados(prev => prev.slice(0, limiteProductos()));
      }
      if (supersActivos.length > limiteSupers()) {
        setSupersActivos(prev => prev.slice(0, limiteSupers()));
      }
    }
