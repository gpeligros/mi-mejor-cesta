"""
revisar_matches_dia.py
======================
Puntúa la calidad de los matches DIA restantes usando Claude API.
Compara nombre_normalizado del catálogo vs nombre_comercial de DIA
y asigna un score de 0 a 10 + motivo.

USO:
  python scrapers/revisar_matches_dia.py              # analiza todos y genera CSV
  python scrapers/revisar_matches_dia.py --dry-run    # solo muestra los primeros 30
  python scrapers/revisar_matches_dia.py --umbral 5   # marca como incorrectos score < 5 (default: 5)

SALIDA:
  matches_dia_revisados.csv  → todos los matches con su score
  matches_dia_incorrectos.csv → solo los que están por debajo del umbral
...
Reglas importantes:
- Si el catálogo es marca_blanca, compara solo el tipo de producto, ignora la marca DIA
...
[
  {"id_catalogo": "CAT-xxxx", "id_dia": "DI-xxxx", "score": 8, "motivo": "mismo producto, distinto formato"},
  ...
]"""
...
def fetch_matches(supabase: Client) -> list:
    """Obtiene todos los matches DIA activos con sus nombres."""
    resultado = supabase.rpc("get_matches_dia_para_revision", {}).execute()
...
def fetch_matches_paginado(supabase: Client) -> list:
    ...
    while True:
        res = (
            ...
            .select("id_catalogo, id_dia")
            .not_.is_("id_dia", "null")
    ...
    ids_dia = list({m["id_dia"] for m in matches_raw})
    ...
    # 3. Obtener datos de precios_dia
    print("  → Cargando precios_dia...")
    precios = {}
    for i in range(0, len(ids_dia), 500):
        res = (
            supabase.table("precios_dia")
            .select("id, nombre_comercial, marca, precio")
            .in_("id", ids_dia[i:i+500])
            .execute()
        )
        for r in res.data:
            precios[r["id"]] = r
...
def normalizar_matches(raw: list) -> list:
    ...
    for r in raw:
        ...
        resultado.append({
            ...
            "nombre_dia": dia.get("nombre_comercial", ""),
            "marca_dia": dia.get("marca", ""),
            "precio_dia": dia.get("precio", ""),
        })
    return resultado
...
def puntuar_lote(cliente: anthropic.Anthropic, matches: list) -> list:
    ...
    for m in matches:
        ...
        lineas.append(
            f"{m['id_catalogo']} | {m['id_dia']} | "
            f"CATÁLOGO ({m['tipo']}): {m['nombre_catalogo']}{marca_info} | "
            f"DIA: {m['nombre_dia']} (marca: {m['marca_dia']})"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Puntúa matches DIA con IA")
    ...
    fichero_todos = f"matches_dia_revisados_{timestamp}.csv"
    fichero_malos = f"matches_dia_incorrectos_{timestamp}.csv"
    ...
    if args.dry_run:
        print(f"\n  ⚠️  DRY-RUN: ejecuta sin --dry-run para procesar los {659} matches completos.")
    else:
