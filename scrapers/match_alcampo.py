"""
match_alcampo.py  —  Mi Mejor Cesta
=================================
Vincula productos_catalogo (CAT-xxxx) con precios_alcampo (AL-xxxx)
usando similitud de nombres con normalización agresiva (rapidfuzz).

Mejoras v3:
- Matching 1-a-1: cada producto Alcampo solo puede asignarse una vez
...
Uso:
  python match_alcampo.py --dry-run   # ver matches sin guardar
  python match_alcampo.py             # guardar matches en Supabase
"""
...
PALABRAS_ELIMINAR = {
    "producto", "alcampo", "seleccion", "selección", "especial",
...
def normalizar(texto: str, es_alcampo: bool = False, marca: str = "") -> str:
    if not texto:
        return ""
    t = texto.lower()
    t = quitar_acentos(t)
...
def calcular_score(nombre_cat: str, nombre_alc: str, marca_alc: str = "") -> float:
    norm_cat = normalizar(nombre_cat)
    norm_alc = normalizar(nombre_alc, es_alcampo=True, marca=marca_alc)

    if not norm_cat or not norm_alc:
        return 0.0

    palabras_cat = set(w for w in norm_cat.split() if len(w) > 4)
    palabras_alc = set(w for w in norm_alc.split() if len(w) > 4)

    # REQUISITO: al menos una palabra clave debe coincidir
    if palabras_cat and palabras_alc:
        if not (palabras_cat & palabras_alc):
            return 0.0

    # PENALIZACIÓN: palabras ambiguas sin contexto adicional
    palabras_cat_todas = set(norm_cat.split())
    palabras_alc_todas = set(norm_alc.split())
    ambiguas_cat = palabras_cat_todas & AMBIGUAS
    if ambiguas_cat:
        no_ambiguas_cat = {p for p in palabras_cat_todas if p not in AMBIGUAS and len(p) > 3}
        no_ambiguas_alc = {p for p in palabras_alc_todas if p not in AMBIGUAS and len(p) > 3}
        if no_ambiguas_cat and not (no_ambiguas_cat & no_ambiguas_alc):
            return 0.0

    score_set  = fuzz.token_set_ratio(norm_cat, norm_alc) / 100
    score_sort = fuzz.token_sort_ratio(norm_cat, norm_alc) / 100
    score_part = fuzz.partial_ratio(norm_cat, norm_alc) / 100
...
def cargar_datos(client):
    ...
    while True:
        ...
        if len(res.data) < 1000: break
        offset += 1000
    ya_matched_cat = {m["id_catalogo"] for m in matches if m.get("id_alcampo")}
    ya_matched_alc = {m["id_alcampo"] for m in matches if m.get("id_alcampo")}
    log.info(f"  {len(ya_matched_cat)} productos ya tienen match con Alcampo")

    return catalogo, alcampo, ya_matched_cat, ya_matched_alc


def hacer_matching(catalogo, alcampo, ya_matched_cat, ya_matched_alc):
    ...
    alcampo_disponible = [a for a in alcampo if a["id"] not in ya_matched_alc]

    log.info(f"\nCalculando scores: {len(pendientes)} catálogo vs {len(alcampo_disponible)} Alcampo...")

    todos_scores = []
    for i, prod in enumerate(pendientes, 1):
        nombre_cat = prod.get("nombre_generico", "") or ""
        for alc in alcampo_disponible:
            s = calcular_score(nombre_cat, alc.get("nombre_comercial", ""), alc.get("marca", ""))
            if s >= SCORE_MIN / 100:
                todos_scores.append({
                    ...
                    "nombre_alcampo":  alc.get("nombre_comercial", ""),
                    "marca_alcampo":   alc.get("marca", ""),
                })
        if i % 200 == 0:
    ...
    usados_cat, usados_alc = set(), set()
    resultados = []

    for par in todos_scores:
        if par["id_catalogo"] in usados_cat or par["id_alcampo"] in usados_alc:
            continue
        resultados.append({**par, "auto": par["score"] >= SCORE_AUTO / 100})
        usados_cat.add(par["id_catalogo"])
        usados_alc.add(par["id_alcampo"])
...
def guardar_matches(client, resultados, solo_auto=False):
    ...
    for i in range(0, len(a_guardar), BATCH_SIZE):
        ...
        updates = [{"id_catalogo": r["id_catalogo"], "id_alcampo": r["id_alcampo"],
                    "id_alcampo_score": r["score"]} for r in batch]
        try:
...
def main(dry_run=False):
    log.info("━" * 55)
    log.info("  MATCHING ALCAMPO v3 → productos_match")
    log.info("━" * 55)

    client = get_supabase()
    catalogo, alcampo, ya_matched_cat, ya_matched_alc = cargar_datos(client)
    resultados = hacer_matching(catalogo, alcampo, ya_matched_cat, ya_matched_alc)

    log.info("\n📋 Top 30 mejores matches:")
    for r in sorted(resultados, key=lambda x: -x["score"])[:30]:
        tag = "✅ AUTO " if r["auto"] else "⚠️  REVISAR"
        log.info(f"  {tag} [{r['score']:.2f}] {r['nombre_catalogo'][:35]:<35} → {r['nombre_alcampo'][:45]}")

    log.info("\n📋 Matches con score más bajo (últimos 20):")
    for r in sorted(resultados, key=lambda x: x["score"])[:20]:
        log.info(f"  ⚠️  [{r['score']:.2f}] {r['nombre_catalogo'][:35]:<35} → {r['nombre_alcampo'][:45]}")
